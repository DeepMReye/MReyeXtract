...
# --------------------------ANTS TRANSFORMS---------------------------------------
# --------------------------------------------------------------------------------
def register_to_eye_masks(
    dme_template, func, masks, verbose=1, transforms=None, metric="GC"
):
    ...
    for idx, mask in enumerate(masks):
        ...
        if verbose > 0:
            if "SyN" in type_of_transform:
                registered_fwd = loadmat(register_to_nau["fwdtransforms"][1])[
                    "AffineTransform_float_3_3"
                ]
            else:
                registered_fwd = loadmat(register_to_nau["fwdtransforms"][0])[
                    "AffineTransform_float_3_3"
                ]
            print(
...
def extract_mask(
    ...
    ----------
    run_paths : RunPaths
        Run paths including input and outputh paths
    dme_template : ants Image
    ...
    func = ants.image_read(str(run_paths.in_path))
...
def _mask_template_paths() -> tuple[str, str, str]:
    """
    Return mask template paths
    ...
    """
    mask_root = files("mreyextract.data") / "masks"

    # Use as_file so we always get a real filesystem Path, even if the package
    # is in a zip / wheel.
    big_res = mask_root / "eyemask_big.nii"
    small_res = mask_root / "eyemask_small.nii"
    tmpl_res = mask_root / "dme_template.nii"

    with (
        as_file(big_res) as big_path,
        as_file(small_res) as small_path,
        as_file(tmpl_res) as tmpl_path,
    ):
        return str(big_path), str(small_path), str(tmpl_path)
...
def get_masks():
    ...
    z_edges : list
        Edges of mask in z-dimension
    """
    eyemask_big_path, eyemask_small_path, dme_template_path = _mask_template_paths()

    print(eyemask_small_path)
    eyemask_small = ants.image_read(eyemask_small_path)
    eyemask_big = ants.image_read(eyemask_big_path)
    dme_template = ants.image_read(dme_template_path)
...
def get_mask_edges(mask: ants.ANTsImage, split=True):
    """Get edges of mask.

    Parameters
    ----------
    mask : filepath, optional
        Filepath to mask
    split : bool, optional
    ...
    if split:
        ...
        x_edges = (
            np.max(left_indices),
            np.min(left_indices),
            np.max(right_indices),
            np.min(right_indices),
        )
    else:
        middle = (
            np.min(edge_indices[0])
            + (np.max(edge_indices[0]) - np.min(edge_indices[0])) // 2
        )
...
def cut_mask(
    ...
    # Slice for mask
    masked_eye_left = to_mask[
        x_edges[1] : x_edges[0], y_edges[1] : y_edges[0], z_edges[1] : z_edges[0], ...
    ]
    masked_eye_right = to_mask[
        x_edges[3] : x_edges[2], y_edges[1] : y_edges[0], z_edges[1] : z_edges[0], ...
    ]
    masked_eye = np.concatenate((masked_eye_right, masked_eye_left))
    if verbose > 0:
        print(
            f"Voxels > 0 / Mean of voxels: {np.sum(np.mean(masked_eye, axis=3) > 0)} / {np.mean(masked_eye)}"
        )
    ...
    if save_overview:
        ...
        plot_subject_report(run_paths.out_report_path, original_input, masked_eye, mask)
    # "sub-03_task-imagery_run-1_desc-bold-mask.nii.gz"
    if as_pickle:
        # fn_masked_eye_path = save_dir / f"mask_{participant_basename}.p"
        pickle.dump(masked_eye, open(run_paths.out_eye_path, "wb"))
    else:
        ...
        ants.image_write(masked_eye_img, str(run_paths.out_eye_path))
...
def plot_subject_report(
    ...
    fig.add_trace(
        go.Heatmap(
            z=whole_brain_mask[25, :, :].transpose(),
            showscale=False,
            colorscale="Greys_r",
        ),
        row=1,
    ...
    fig.add_trace(
        go.Heatmap(
            z=whole_brain_mask[:, 90, :].transpose(),
            showscale=False,
            colorscale="Greys_r",
        ),
        row=1,
    ...
    fig.add_trace(
        go.Heatmap(z=whole_brain_mask[:, :, 15], showscale=False, colorscale="Greys_r"),
        row=1,
        col=3,
    )
    fig.add_trace(
    ...
    fig.add_trace(
        go.Heatmap(
            z=np.mean(eye_mask, axis=0).transpose(), showscale=False, colorscale="Hot"
        ),
        row=2,
        col=1,
    )
    fig.add_trace(
        go.Heatmap(
            z=np.mean(eye_mask, axis=1).transpose(), showscale=False, colorscale="Hot"
        ),
        row=2,
        col=2,
    )
    fig.add_trace(
        go.Heatmap(z=np.mean(eye_mask, axis=2), showscale=False, colorscale="Hot"),
        row=2,
        col=3,
    )
    fig.add_trace(
        go.Scatter(
            x=np.arange(0, len(eye_mask_timecourse)),
    ...
    fig.add_trace(
        go.Scatter(
            x=np.arange(0, len(whole_brain_timecourse)),
...
def save_data(
    ...
    participant_data = np.transpose(
        np.concatenate(participant_data, axis=3), axes=(3, 0, 1, 2)
    )
